package com.ting.sonik.ui.viewmodels
...
import com.ting.sonik.tools.MusicProvider
import com.ting.sonik.tools.Song
import com.ting.sonik.tools.SettingsManager
import com.ting.sonik.tools.MetadataManager
...
class MusicViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // PLAYLISTS
    var playlists by mutableStateOf<List<com.ting.sonik.data.Playlist>>(emptyList())
        private set

    var playlistMappings by mutableStateOf<List<com.ting.sonik.data.PlaylistSong>>(emptyList())
        private set

    var topSongStats by mutableStateOf<List<com.ting.sonik.data.PlaybackStats>>(emptyList())
        private set
    var topPlaylistStats by mutableStateOf<List<com.ting.sonik.data.PlaybackStats>>(emptyList())
        private set
    var topArtistStats by mutableStateOf<List<com.ting.sonik.data.PlaybackStats>>(emptyList())
        private set

    fun loadPlaylists() {
        viewModelScope.launch {
            val (newPlaylists, newMappings) = withContext(Dispatchers.IO) {
                val db = com.ting.sonik.data.MusicDatabase.getDatabase(getApplication())
    ...
    fun createPlaylist(name: String) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().insertPlaylist(
                    com.ting.sonik.data.Playlist(name = name)
                )
            }
            loadPlaylists()
        }
    ...
    fun addSongToPlaylist(playlistId: Long, songId: Long, onComplete: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().addSongToPlaylist(
                    com.ting.sonik.data.PlaylistSong(playlistId, songId)
                )
            }
            awaitLoadPlaylists()
            onComplete?.invoke()
    ...
    fun addSongsToPlaylist(playlistId: Long, songIds: List<Long>, onComplete: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                val db = com.ting.sonik.data.MusicDatabase.getDatabase(getApplication())
                val playlistSongs = songIds.map { com.ting.sonik.data.PlaylistSong(playlistId, it) }
    ...
    fun removeSongFromPlaylist(playlistId: Long, songId: Long, onComplete: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().removeSongFromPlaylist(playlistId, songId)
            }
            awaitLoadPlaylists()
            onComplete?.invoke()
        }
    ...
    fun getPlaylistsContainingSong(songId: Long, callback: (List<Long>) -> Unit) {
        viewModelScope.launch {
            ...
            val all = withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().getAllPlaylists()
            }
            val containingIds = mutableListOf<Long>()
            withContext(Dispatchers.IO) {
                val dao = com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao()
                for (playlist in all) {
    ...
    fun getSongsForPlaylist(playlistId: Long, callback: (List<Song>) -> Unit) {
        viewModelScope.launch {
            val ids = withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().getSongIdsForPlaylist(playlistId)
            }
    ...
    fun deletePlaylist(playlist: com.ting.sonik.data.Playlist, onComplete: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                com.ting.sonik.data.MusicDatabase.getDatabase(getApplication()).playlistDao().deletePlaylist(playlist)
            }
            loadPlaylists()
            onComplete?.invoke()
        }
        ...
        viewModelScope.launch {
            val deleted = withContext(Dispatchers.IO) {
                try {
                    ...
                    if (success) {
                        val db = com.ting.sonik.data.MusicDatabase.getDatabase(context)
